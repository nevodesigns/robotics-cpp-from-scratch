...
//
// Measured with two atomic counters and two threads, four million increments
// each: 159 ms when the counters were 8 bytes apart, 139 ms at 16, and 27 ms
// once they crossed a line boundary. Nothing changes until they cross it, and
...
// a std::vector<std::atomic<long>> took 162 ms; one slot per cache line took
// 35; a local on the stack, published once at the end, took 8.
//
