// rc/kin/reach.hpp
//
// The numerical solver from lesson 13-03, graduated.
//
// A closed form exists for the arms that were designed to have one. This works
// on the rest, including an arm whose geometry came out of a file this morning,
// because it never needs to know what shape the arm is: it measures how the
// tool moves when each joint moves, and steps in the direction that helps.
//
// The step is the transpose of the Jacobian applied to the error, so there is
// no matrix inverse anywhere. It is the simplest method that works and it is
// not the fastest; what it buys is that every line of it can be read.
//
// Three numbers from that lesson are worth carrying. The finite difference step
// has a best value near 1e-6, and 1e-14 is worse than 1e-1, because truncation
// and rounding move in opposite directions. Answers agree with the closed form
// to about 5e-9 radians. And convergence slows sharply toward the edge of the
// workspace, 390 iterations then 1281 then 5236, before stalling entirely,
// which is the singularity seen from the solver's side rather than the
// geometry's.

#ifndef RC_KIN_REACH_HPP
#define RC_KIN_REACH_HPP
