// rc/rt/histogram.hpp
//
// The instrument from lesson 07-03, graduated.
//
// Every timing claim in this curriculum is produced with this, because an
// average over a timing distribution is close to useless: a loop cannot run
// much earlier than its period, since it waits, but it can run arbitrarily
// later, since something else took the processor. The distribution has a hard
// floor and a long tail, and the tail is the entire question.
//
// Fixed memory decided up front, so recording allocates nothing inside the loop
// being measured. Overflows counted separately and the worst kept as a measured
// value rather than a bucket edge, so the histogram cannot understate its own
// tail, which is the failure catalogued as E-RT-0001.

#ifndef RC_RT_HISTOGRAM_HPP
#define RC_RT_HISTOGRAM_HPP
