// rc/plot/series.hpp
//
// The rolling series and value axis from lesson 10-02, graduated.
//
// What separates a chart that helps from one that misleads is two decisions,
// and neither is about drawing.
//
// A rolling window is bounded by age and guarded by capacity. Bounded by count
// alone, the same code shows fifty seconds of history at ten hertz and half a
// second at a thousand, and two runs of the same signal cannot be compared.
//
// And a value axis needs a floor under its span. Every real measurement moves
// in its last digit, and an axis fitted to that expands until the wobble fills
// the chart: measured, a battery reading steady to within two microvolts
// occupies more than fifteen rows of twenty. The minimum span is a statement
// about what counts as a change, which the chart cannot know and its author
// can.

#ifndef RC_PLOT_SERIES_HPP
#define RC_PLOT_SERIES_HPP
