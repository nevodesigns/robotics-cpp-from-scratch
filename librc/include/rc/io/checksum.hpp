// rc/io/checksum.hpp
//
// The integrity checks from lesson 08-03, graduated.
//
// Which one to use is decided by two rules the lesson measured rather than
// quoted.
//
// Below the width of the checksum, a CRC is a guarantee: a CRC of width n
// detects every burst of n bits or fewer, by construction. Above that width it
// is chance at one in two to the n, and there every scheme of the same width
// performs identically. Over three million messages with runs of bytes replaced
// by noise, a sum, an exclusive or and CRC-8 each missed about one in 256.
//
// And a sum or an exclusive or catches exactly none of a reordering, because
// both are commutative. That matters here because a frame parser that
// resynchronised onto a false start byte delivers correct bytes in the wrong
// places, which is the one thing they cannot see.
//
// Each CRC carries its published check value, the result over the nine bytes
// "123456789", because round tripping against your own decoder passes for every
// wrong variant of the algorithm.

#ifndef RC_IO_CHECKSUM_HPP
#define RC_IO_CHECKSUM_HPP
