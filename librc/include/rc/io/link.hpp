// rc/io/link.hpp
//
// The link from lesson 08-04, graduated.
//
// Everything needed to talk to a device except the device. The platform
// specific part is small and lives behind BytePort; everything above it is the
// same code for a serial port, a socket, a USB device or a radio, and all of it
// is testable without hardware.
//
// Three things here are not obvious and each of them is a defect somebody has
// shipped.
//
// A read of zero is not an error and not an end. On a link polled faster than
// it talks it is most calls.
//
// A write is a request rather than an instruction, and finishing it takes a
// loop. A truncated frame is worse than a missing one, because the far end
// loses its place in the stream rather than one message.
//
// Staleness is measured from the last valid message and from nothing else. A
// crashed board holding its line produces bytes all day, and a watchdog fed by
// bytes reports a healthy link most confidently when things are worst.

#ifndef RC_IO_LINK_HPP
#define RC_IO_LINK_HPP
  ...
  bool poll(double now) {
    ...
    for (int attempt = 0; attempt < kMaxReadsPerPoll; ++attempt) {
      ...
      while (position_ < filled_) {
        ...
        if (!verify_and_strip(parser_.payload(), body)) {
