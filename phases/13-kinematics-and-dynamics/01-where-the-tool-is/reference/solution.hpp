...
class Chain {
 public:
  ...
  rc::math::Transform frame_at(std::size_t index, rc::span<const double> angles) const {
    rc::math::Transform base_here = identity();
    ...
    for (std::size_t i = 0; i <= last && i < joints_.size(); ++i) {
      ...
      base_here = rc::math::compose(base_here, joints_[i].parent_to_joint);
      ...
      base_here = rc::math::compose(base_here, joint_motion(joints_[i], angle));
    }
    return base_here;
  }
