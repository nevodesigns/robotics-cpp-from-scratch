// rc/kin/planar_arm.hpp
//
// The closed form solver from lesson 13-02, graduated.
//
// Forward kinematics has one answer. Inverse kinematics has a set, and that is
// the shape of every inverse problem in robotics rather than a quirk of this
// arm. A planar two link arm reaching a point almost always has two answers, a
// six axis arm has up to eight, a redundant arm has infinitely many, and in
// every case choosing among them is a decision about the world that belongs to
// the caller.
//
// Two numbers from that lesson are worth carrying with the code. Driving 992
// solutions back through the forward kinematics misses by at most 3.1e-16
// metres. And at full extension the law of cosines produces an argument of
// about 1 + 7e-16: measured over a hundred thousand fully extended
// configurations, more than half exceeded one, which is why the clamp is here
// and is not defensive noise.

#ifndef RC_KIN_PLANAR_ARM_HPP
#define RC_KIN_PLANAR_ARM_HPP
