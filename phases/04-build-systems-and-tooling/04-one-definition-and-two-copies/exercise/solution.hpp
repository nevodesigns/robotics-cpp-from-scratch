...
class Registry {
 public:
  ...
  // TODO 1: the one instance.
  //
  // Return a reference to a function-local static.
  //
  // That is the header-only way to have exactly one of something. The function
  // is inline because a member function defined inside the class is, which
  // makes it one entity across the whole binary, and the standard then
  // guarantees its local static is one object, constructed once, however many
  // translation units call it.
  //
  // A namespace-scope variable in a header cannot do this. Without inline it is
  // a definition per file and the link fails; with static it is a separate
  // object per file, which is the fault this class exists to avoid and which
  // the tests measure.
  ...
  // TODO 2: add an entry, or refuse one.
  //
  // Push the name and the pointer onto entries_. A null name or a null value is
  // nothing to store, so return without adding.
  ...
  // TODO 3: the value registered under a name, or nothing.
  //
  // Walk the entries and compare the names with std::strcmp, not with ==.
  //
  // Two translation units that both write "wheel_base" are not required to
  // produce the same address for it, so comparing pointers finds the entry
  // sometimes, on some compilers, at some optimisation levels. The test hands
  // in a name spelled out character by character to make sure.
  //
  // Return nullptr for a name that is not there, and for a null name.
