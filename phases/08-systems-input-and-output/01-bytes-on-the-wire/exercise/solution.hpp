...
class ByteWriter {
 public:
  ...
  bool put_u16(std::uint16_t value) {
    ...
    // TODO: write the two bytes, most significant first.
    //
    // Shift the value, do not copy its memory. A shift is arithmetic on a
    // number and means the same thing on every machine; copying the memory
    // means whatever this processor's byte order happens to be, which the
    // device at the other end does not have to share.
    (void)value;
    return true;
  }
  ...
  bool put_f32(float value) {
    // TODO: put the float's bits on the wire.
    //
    // Get at them with memcpy, not with reinterpret_cast. The cast is
    // undefined behaviour, and on a pointer into the middle of a buffer it is
    // also misaligned, which faults on some ARM cores. Every compiler turns
    // this memcpy into the same instruction the cast would have produced.
    (void)value;
    return false;
  }
...
class ByteReader {
 public:
  ...
  bool get_u32(std::uint32_t& out) {
    // TODO: check there are four bytes left, then read them most significant
    // first. Leave out untouched and set ok_ to false when there are not
    // enough: a decoder that invents a value is worse than one that fails,
    // because the value looks like data.
    (void)out;
    return false;
  }
  ...
  bool get_i32(std::int32_t& out) {
    // TODO: read the four bytes and turn them back into a signed value.
    //
    // What travels is the two's complement bit pattern, so what comes back is
