// rc/control/feedforward.hpp
//
// What the plant needs before the error is even measured, from lesson 14-05.
//
...
// at 0.5 m/s with kp = 20 and kd = 8:
//
//   feedback only, derivative on measurement    0.234000 m
//   with feedforward for the plant              0.199000 m
//   derivative on error instead                 0.034000 m
//   both                                        0.001000 m
//
// Two causes, cleanly separable. The plant's own force costs (load + damping*v)
// / kp = 0.035, and the derivative term opposing steady motion costs kd*v / kp
// = 0.200. Feedforward removes the first, taking the derivative on the error
// removes the second, and neither removes the other.
//
// Taking the derivative on the error has its own price, which is why it is not
// the default: on a one metre step it produced 4020 N against 20 N.

#ifndef RC_CONTROL_FEEDFORWARD
#define RC_CONTROL_FEEDFORWARD
