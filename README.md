...
```text
86 lessons   16 phases   206 atlas entries   4 toolchains claimed   178 hours of material
```
<!-- COUNTS:END -->
...
<!-- PHASES:START (generated by rcpp readme, do not edit by hand) -->
| Phase | Lessons | Ends with |
|---|---|---|
| 00 Toolchain and First Motion | 8 | Your robot's quarter circle, drawn in the terminal with nothing installed, and in a window if you want it |
| 01 C++ Core I | 7 | A drive state machine that cannot represent nonsense |
| 02 Memory and Ownership | 6 | A subsystem tree that releases completely, with no cycles |
| 03 C++ Core III | 6 | An injectable clock, so dt finally has a provenance |
| 04 Build Systems and Tooling | 4 | A library of your own, installed, and consumed by a project that has never seen its source |
| 05 Testing and Fakes | 5 | A test suite you can prove catches bugs, because you watched it catch six |
| 06 Robot Maths | 6 | Where the gripper is, in the world frame, and how you know |
| 07 Concurrency and Real Time | 5 | A measured latency histogram of your own control loop |
| 08 Systems Input and Output | 5 | A framed and checked conversation with a device, over a port you opened |
| 09 Qt Core | 4 | A sensor on its own thread, reaching the interface without ever touching it |
| 10 Qt Widgets | 4 | A live chart of any signal, in a window and in a terminal, that does not lie about a steady one |
| 11 Qt Quick and QML | 2 | A QML interface whose failures are visible from a test rather than from a console nobody is watching |
| 13 Kinematics and Dynamics | 5 | An arm asked its question in both directions, and told apart the pose it cannot reach from the one it cannot hold |
| 14 Control | 6 | A loop you can tune, with four measured numbers rather than a feeling |
| 15 Sensors, Drivers and Vision | 6 | A noisy sensor made usable, with the cost of making it so measured rather than guessed, and a camera frame read the way it was actually laid out |
| 16 Estimation and Navigation | 7 | A rover that plans a route through what it has actually seen, drives it, fits down it, and notices when the room changes |
<!-- PHASES:END -->
