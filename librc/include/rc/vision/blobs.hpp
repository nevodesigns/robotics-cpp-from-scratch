// rc/vision/blobs.hpp
//
// A threshold taken from the picture rather than from a constant, and what
// counts as one thing, from lesson 15-06.
//
// A fixed threshold is a statement about the lighting, and lighting is the one
// thing about a scene guaranteed to change. Measured on a marker at 200 against
// a background at 40:
//
...
//
// At the last row the background itself is above 128, so the fixed threshold
// selects the whole frame. The marker is 20 pixels.
//
// Otsu answers where to split and not whether there is anything to split. On an
// evenly lit wall it returns 0 and every pixel is above it, so the answer is not
// nothing but everything: one blob covering the frame, centred in the middle of
// the picture. Something after it has to decide, and the cheapest test is how
// much of the frame came back.

#ifndef RC_VISION_BLOBS
#define RC_VISION_BLOBS
