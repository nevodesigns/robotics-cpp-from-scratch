// rc/core/units.hpp
//
// A number that knows what it is, from lesson 03-06.
//
...
// radians and degrees, seconds and milliseconds, a position and a velocity.
...
// Quantity<MetreTag> and Quantity<SecondTag> are different types, which is the
// whole mechanism: the arithmetic is defined between two of the same tag and
// nowhere else, so the compiler refuses the rest. It costs nothing at runtime:
// sizeof is one double and every operation is a function that disappears.
//
// Three snippets in that lesson's checks/ directory are compiled by the build
// and expected to fail: adding a length to a duration, passing a bare double
// where a length was wanted, and multiplying two lengths. A guarantee the
// compiler makes has to be checked by the compiler.
//
// What it does not catch is worth knowing too. Two lengths of wildly different
// kinds are the same type, so adding a person's height to a wavelength is
// arithmetic this has no opinion about, and an angle of 370 degrees is a
// perfectly good Radians. A unit is not a meaning.

#ifndef RC_CORE_UNITS
#define RC_CORE_UNITS
