// rc/io/bytes.hpp
//
// The encoder and decoder from lesson 08-01, graduated.
//
// Everything this curriculum sends anywhere goes through these. A struct does
// not: sizeof includes padding your compiler chose for your processor, and the
// device at the other end chose differently. Fields go, one at a time, in a
// width and an order the protocol specifies.
//
// Big endian on the wire, because most sensor and network protocols are, and
// because a packet capture then reads left to right in the order the datasheet
// describes. The bytes are produced by shifting the value rather than by
// copying its memory, so the code has no endianness of its own and means the
// same thing wherever it is compiled.
//
...
#ifndef RC_IO_BYTES_HPP
#define RC_IO_BYTES_HPP
